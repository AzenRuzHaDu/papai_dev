...
```
feat(auth): add JWT refresh token endpoint [gemini/dev]
fix(booking-service): handle overlapping time slots [gemini/dev]
docs(architecture): add DTO decision rationale [gemini/architect]
test(user-repo): add edge case for duplicate email [gemini/tester]
...
```
fix(payment): prevent double charge on retry [gemini/dev]
